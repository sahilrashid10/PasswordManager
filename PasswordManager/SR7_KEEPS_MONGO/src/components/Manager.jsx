...
const Manager = () => {
    ...
    const handleEdit = async id => {
        ...
        // Pre-fill the form with the selected item
        setForm({ site: item.site, username: item.username, password: item.password });

        // Remove the old item from UI for now
        const updatedArray = passwordArray.filter(item => item.id !== id);
        setPasswordArray(updatedArray);

        // Send just the item to update (ideally after editing is complete, not here)
        let res = await fetch("http://localhost:3000", {
            method: "PUT",
            headers: {
                "Content-Type": "application/json"
            },
            body: JSON.stringify({ id, ...item })  // ✅ Send the item being edited
        });
    };
